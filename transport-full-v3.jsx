...
// ── 請求管理 ───────────────────────────────────────
function InvoicePrint({bill, company}){
  // 印刷用請求書（別ウィンドウで開く）
  const html=`<!DOCTYPE html>
<html lang="ja">
<head><meta charset="UTF-8">
<title>請求書 ${bill.no}</title>
<style>
  body{font-family:'MS Gothic','Hiragino Kaku Gothic Pro',sans-serif;margin:0;padding:40px;color:#1a1a1a;font-size:13px}
  .title{font-size:28px;font-weight:bold;text-align:center;margin-bottom:8px;letter-spacing:0.2em}
  .subtitle{text-align:center;color:#555;margin-bottom:40px;font-size:12px}
  .flex{display:flex;justify-content:space-between;margin-bottom:32px}
  .to-block{flex:1}
  .to-name{font-size:18px;font-weight:bold;border-bottom:2px solid #1a1a1a;padding-bottom:6px;margin-bottom:4px}
  .info-block{text-align:right;font-size:12px;line-height:1.8}
  ...
  .total-amount{font-size:18px;font-weight:bold;text-align:right;margin:16px 0;padding:12px;background:#1a1a1a;color:#fff}
  .note{margin-top:24px;padding:12px;background:#f9f9f9;border-left:3px solid #aaa;font-size:12px;color:#555}
  .bank-info{margin-top:20px;padding:14px;border:1px solid #ddd;border-radius:4px}
  .bank-title{font-weight:bold;margin-bottom:6px}
  @media print{body{padding:20px}.no-print{display:none}}
</style>
</head>
<body>
<div class="no-print" style="text-align:center;margin-bottom:20px">
  <button onclick="window.print()" style="background:#f0a500;color:#000;border:none;padding:10px 28px;font-size:15px;font-weight:bold;border-radius:4px;cursor:pointer">🖨 印刷する</button>
</div>
<div class="title">請 求 書</div>
<div class="subtitle">${bill.no}</div>
<div class="flex">
  <div class="to-block">
    <div class="to-name">${bill.customer}&nbsp;御中</div>
    <div style="font-size:12px;color:#555;margin-top:6px">下記の通りご請求申し上げます。</div>
    <div style="margin-top:16px;font-size:13px">
      <strong>お支払期限：</strong>${bill.dueDate||"--"}
    </div>
  </div>
  <div class="info-block">
    <div class="company-name">${company?.name||"株式会社〇〇運送"}</div>
    <div>${company?.address||"〒000-0000 大阪府〇〇市〇〇町1-1"}</div>
    <div>TEL: ${company?.phone||"06-0000-0000"}</div>
    <div>登録番号: ${company?.regNo||"T0000000000000"}</div>
    <div style="margin-top:8px">発行日: ${bill.issueDate||"--"}</div>
  </div>
</div>
<table>
  <thead>
  ...
  <tbody>
    <tr><td>運送料金（${bill.note||"配送サービス"}）</td><td class="amount">¥${Number(bill.amount).toLocaleString()}</td></tr>
    <tr class="total-row"><td>小計</td><td class="amount">¥${Number(bill.amount).toLocaleString()}</td></tr>
    <tr><td>消費税（10%）</td><td class="amount">¥${Number(bill.tax).toLocaleString()}</td></tr>
  </tbody>
</table>
<div class="total-amount">合計金額　¥${(Number(bill.amount)+Number(bill.tax)).toLocaleString()}</div>
${bill.note?`<div class="note">備考：${bill.note}</div>`:""}
<div class="bank-info">
  <div class="bank-title">お振込先</div>
  <div>${company?.bankName||"〇〇銀行 〇〇支店"}</div>
  <div>口座種別：${company?.bankType||"普通"}&nbsp;&nbsp;口座番号：${company?.bankNo||"0000000"}</div>
  <div>口座名義：${company?.bankHolder||"カブシキガイシャ〇〇ウンソウ"}</div>
</div>
</body></html>`;
...
function Billing({data,setData}){
  ...
  const [company,setCompany]=useState(
    JSON.parse(localStorage.getItem("tms-company")||"{}")
  );
  ...
  const saveCompany=(c)=>{
    localStorage.setItem("tms-company",JSON.stringify(c));
    setCompany(c);
    ...
    <div>
      <PageHd title="請求管理" action={
        <div style={{display:"flex",gap:8}}>
          <Btn v="ghost" onClick={()=>setShowCompany(true)} sm>自社情報設定</Btn>
          ...
          <tbody>
            ...
            {sorted.map(b=>(
              <tr key={b.id} style={{borderBottom:`1px solid ${C.bdr}`}}>
                ...
                <Td style={{fontSize:11,color:daysUntil(b.dueDate)<=0&&b.status!=="入金済"?C.red:C.txt}}>{fmtDate(b.dueDate)}</Td>
                <Td>
                ...
                <Td>
                  <div style={{display:"flex",gap:4}}>
                    ...
                    <Btn sm v="blue" onClick={()=>InvoicePrint(b, company)}>印刷</Btn>
  ...
  return(
    <Modal title="自社情報設定（請求書に印刷）" onClose={onClose} wide>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
        ...
        <Inp label="インボイス登録番号" value={f.regNo||""} onChange={e=>s("regNo",e.target.value)} style={{gridColumn:"span 2"}}/>
      </div>
      <Inp label="住所" value={f.address||""} onChange={e=>s("address",e.target.value)}/>
...
function OpLogs({data,setData,role,cDrvId}){
  ...
  return(
    <div>
      <PageHd title="運行履歴記録" action={<Btn onClick={()=>setModal({})}>＋ 登録</Btn>}/>
