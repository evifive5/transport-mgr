...
// ── Tokens ──────────────────────────────────────────────────
const C = {
  bg:"#f0f2f5",s1:"#ffffff",s2:"#f8f9fb",s3:"#eef0f3",
  bdr:"#e0e4ea",bdr2:"#c8cdd8",
  acc:"#d97706",accD:"#fef3c7",
  blue:"#2563eb",blueD:"#eff6ff",
  grn:"#16a34a",grnD:"#f0fdf4",
  red:"#dc2626",redD:"#fef2f2",
  ylw:"#b45309",ylwD:"#fffbeb",
  pur:"#7c3aed",purD:"#f5f3ff",
  txt:"#1e293b",mut:"#64748b",dim:"#cbd5e1",
};
...
const SEED = {
  drivers:[
    {id:"d1",no:"DRV001",name:"田中 一郎",kana:"タナカ イチロウ",phone:"090-1234-5678",
     ...
     address:"大阪市北区梅田1-1",birth:"1985-03-15",status:"待機中",pin:"1234",note:""},
...
// ── UI Components ────────────────────────────────────────────
const inputSt = {
  width:"100%",background:"#fff",border:`1px solid ${C.bdr}`,borderRadius:4,
...
function DrvModal({drv,onSave,onClose}){
  const [f,setF]=useState(drv||{name:"",kana:"",phone:"",license:"普通第一種",licenseNo:"",
    licenseExpiry:"",hireDate:"",group:"A班",empType:"正社員",address:"",birth:"",status:"待機中",pin:"",note:""});
  const s=(k,v)=>setF(x=>({...x,[k]:v}));
  return(
    <Modal title={drv?"ドライバー編集":"ドライバー追加"} onClose={onClose} wide>
      ...
      <div style={{background:C.s2,borderRadius:8,padding:"10px 14px",marginBottom:12}}>
        <div style={{color:C.mut,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:6}}>PINコード設定</div>
        <Inp label="PINコード（4桁）空欄=PIN不要" value={f.pin||""} onChange={e=>s("pin",e.target.value.replace(/[^0-9]/g,"").slice(0,4))} placeholder="例: 1234"/>
        <div style={{color:C.mut,fontSize:11}}>設定するとドライバーログイン時に入力が必要になります</div>
      </div>
      <Inp label="備考" value={f.note} onChange={e=>s("note",e.target.value)}/>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:4}}>
        <Btn v="ghost" onClick={onClose}>キャンセル</Btn>
...
function RoleSelect({onSelect,drivers}){
  ...
  const ADMIN_PASS="admin1234"; // 管理者パスワード（DrvModalから変更可）

  const loginDriver=()=>{
    const drv=drivers.find(d=>d.id===did);
    if(drv?.pin&&drv.pin!==pin){setPinErr("PINコードが違います");return;}
    setPinErr("");
    onSelect("driver",did);
  };
  const loginAdmin=()=>{
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",
      ...
      <div style={{width:"100%",maxWidth:420}}>
        ...
        {/* ドライバーログイン */}
        <div style={{background:C.s1,border:`2px solid ${C.bdr}`,borderRadius:16,
          padding:"24px 20px",marginBottom:14,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
            <span style={{fontSize:32}}>🚗</span>
            <div style={{color:C.txt,fontWeight:900,fontSize:20}}>ドライバー</div>
          </div>
          <label style={{color:C.mut,fontSize:14,display:"block",marginBottom:6}}>名前を選択</label>
          <select value={did} onChange={e=>{setDid(e.target.value);setPin("");setPinErr("");}}
            style={{...inputSt,fontSize:17,padding:"12px",marginBottom:12,borderRadius:10,border:`1px solid ${C.bdr}`}}>
            {drivers.map(d=><option key={d.id} value={d.id}>{d.name}</option>)}
          </select>
          {drivers.find(d=>d.id===did)?.pin&&(
            <div style={{marginBottom:12}}>
              <label style={{color:C.mut,fontSize:13,display:"block",marginBottom:6}}>PINコード（4桁）</label>
              <input type="password" inputMode="numeric" maxLength={4} value={pin}
                onChange={e=>{setPin(e.target.value);setPinErr("");}}
                placeholder="・・・・"
                style={{...inputSt,fontSize:22,letterSpacing:"0.3em",padding:"12px",
                  borderRadius:10,textAlign:"center",border:`1px solid ${pinErr?C.red:C.bdr}`}}/>
              {pinErr&&<div style={{color:C.red,fontSize:13,marginTop:4}}>{pinErr}</div>}
            </div>
          )}
          {!drivers.find(d=>d.id===did)?.pin&&(
            <div style={{color:C.mut,fontSize:12,marginBottom:12}}>
              ※ PIN未設定（管理者がドライバー編集画面でPINを設定できます）
            </div>
          )}
          <button onClick={loginDriver} style={{
            width:"100%",background:C.acc,color:"#fff",border:"none",
            borderRadius:12,padding:"15px",fontSize:17,fontWeight:900,cursor:"pointer",
        ...
        {/* 管理者ログイン */}
        <div style={{background:C.s1,border:`1px solid ${C.bdr}`,borderRadius:12,
          padding:"16px 20px",boxShadow:"0 1px 6px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
            <span style={{fontSize:22}}>🖥</span>
            <div style={{color:C.txt,fontSize:15,fontWeight:700}}>管理者ログイン</div>
            <span style={{color:C.mut,fontSize:12,marginLeft:"auto"}}>PC推奨</span>
          </div>
          <input type="password" value={adminPass} onChange={e=>{setAdminPass(e.target.value);setAdminErr("");}}
            placeholder="パスワード（初期: admin1234）"
            style={{...inputSt,marginBottom:8,border:`1px solid ${adminErr?C.red:C.bdr}`}}
            onKeyDown={e=>e.key==="Enter"&&loginAdmin()}/>
          {adminErr&&<div style={{color:C.red,fontSize:12,marginBottom:8}}>{adminErr}</div>}
          <button onClick={loginAdmin} style={{
            width:"100%",background:C.s2,color:C.txt,border:`1px solid ${C.bdr}`,
...
function Sidebar({tab,setTab,role,onLogout,driverName,data}){
  ...
  return(
    <nav style={{width:188,background:C.s1,borderRight:`1px solid ${C.bdr}`,display:"flex",flexDirection:"column",
      flexShrink:0,height:"100vh",overflowY:"auto",boxShadow:"1px 0 6px rgba(0,0,0,0.05)"}}>
...
// ── データエクスポート/インポート ────────────────────────
function DataSyncModal({data, setData, onClose}){
  const exportData=()=>{
    const json=JSON.stringify(data, null, 2);
    const blob=new Blob([json], {type:"application/json"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url; a.download="transport_data_"+todayStr()+".json"; a.click();
  ...
  const importData=(e)=>{
    ...
    reader.onload=(ev)=>{
      try{
        ...
        if(!imported.drivers||!imported.orders) throw new Error("不正なデータ形式");
  ...
  return(
    <Modal title="データ同期（PC↔スマホ）" onClose={onClose}>
      <Alert type="info">
        PCとスマホはデータを自動同期しません。JSONファイルでエクスポート→インポートすることで手動同期できます。
      </Alert>
      <div style={{display:"flex",flexDirection:"column",gap:12,marginTop:8}}>
        <div style={{background:C.s2,borderRadius:10,padding:"16px"}}>
          <div style={{color:C.txt,fontWeight:700,fontSize:14,marginBottom:6}}>① データをエクスポート（バックアップ）</div>
          <div style={{color:C.mut,fontSize:13,marginBottom:10}}>PCで操作したデータをJSONファイルで保存します</div>
          <Btn full onClick={exportData}>📤 データをエクスポート</Btn>
        </div>
        <div style={{background:C.s2,borderRadius:10,padding:"16px"}}>
          <div style={{color:C.txt,fontWeight:700,fontSize:14,marginBottom:6}}>② データをインポート（復元・同期）</div>
          <div style={{color:C.mut,fontSize:13,marginBottom:10}}>エクスポートしたJSONファイルを読み込みます（上書き）</div>
          <label style={{display:"block",background:C.blue+"22",border:`1px solid ${C.blue}44`,
...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      // ログイン状態を復元
      try{
        const sess=JSON.parse(localStorage.getItem("tms-session")||"null");
        if(sess?.role){
          setRole(sess.role);
          setCDrvId(sess.cDrvId||null);
          setTab(sess.tab||"dashboard");
        }
      }catch{}
      setReady(true);
    })();
  ...
  // セッション保存
  useEffect(()=>{
    if(role) localStorage.setItem("tms-session",JSON.stringify({role,cDrvId,tab}));
    else localStorage.removeItem("tms-session");
  },[role,cDrvId,tab]);

  // 手動更新（ログインしたままデータだけ再読込）
  ...
  const doLogout=()=>{ setRole(null); setCDrvId(null); setTab("dashboard"); };

  if(!ready) return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{color:C.acc,fontSize:15,fontWeight:700}}>読み込み中...</div>
    </div>
  );
      ...
      <div style={{background:C.bg, minHeight:"100vh",
        ...
        <div style={{background:C.s1, borderBottom:`1px solid ${C.bdr}`,
          ...
          <div style={{display:"flex",gap:6}}>
            <button onClick={handleRefresh} title="データを更新" style={{
              background:C.s2, border:`1px solid ${C.bdr}`, borderRadius:8,
              padding:"6px 10px", color:C.mut, fontSize:16, cursor:"pointer",lineHeight:1}}>↻</button>
            <button onClick={doLogout} style={{
              background:"none", border:`1px solid ${C.bdr}`, borderRadius:8,
              padding:"6px 10px", color:C.mut, fontSize:13, cursor:"pointer"}}>ログアウト</button>
          </div>
        </div>
        ...
        {/* ボトムナビ */}
        <BottomNav tab={tab} setTab={(t)=>{setTab(t);}} data={data} cDrvId={cDrvId}/>
        {showSync&&<DataSyncModal data={data} setData={setData} onClose={()=>setShowSync(false)}/>}
      </div>
    );
  }
  ...
  return(
    <div style={{display:"flex",height:"100vh",background:C.bg,
      ...
      <Sidebar tab={tab} setTab={(t)=>setTab(t)} role={role} onLogout={doLogout} driverName={driverName} data={data}/>
      <main style={{flex:1,overflow:"auto",padding:24}}>
        {/* 管理者ヘッダーバー */}
        <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginBottom:16}}>
          <button onClick={handleRefresh} title="データを更新" style={{
            background:C.s1, border:`1px solid ${C.bdr}`, borderRadius:6,
            padding:"6px 14px", color:C.mut, fontSize:13, cursor:"pointer",
            display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:16}}>↻</span> 更新
          </button>
          <button onClick={()=>setShowSync(true)} style={{
            background:C.s1, border:`1px solid ${C.bdr}`, borderRadius:6,
            padding:"6px 14px", color:C.mut, fontSize:13, cursor:"pointer",
            display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontSize:14}}>⇄</span> データ同期
          </button>
        </div>
